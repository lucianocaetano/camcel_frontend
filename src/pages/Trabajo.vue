        ...
        <q-list bordered>
          <q-item>
            <q-item-section class="col-12 text-center">
              <q-btn color="green" @click="dialogVisible = true" class="col-12"
                >Registrar trabajo</q-btn
              >
            </q-item-section>
          </q-item>
        </q-list>
                ...
                <q-card-section>
                  ...
                  <div v-if="step === 0">
                    ...
                    <div class="row">
                      <div>
                        <q-btn
                          label="Agregar Fecha y Horario"
                          @click="addFechaHora"
                          color="primary"
                        />
                        <div
                          v-for="(item, index) in fechaHoraList"
                          :key="index"
                  ...
                  <div v-if="step === 1">
                    ...
                    />
                    <div
                      v-for="(operador, index) in operadoresList"
                      :key="index"
                      class="operador-input"
                    >
                  ...
                  <div v-if="step === 2">
                    ...
                    />
                    <div
                      v-for="(doc, index) in documentosList"
                      :key="index"
...
const validarDatos = () => {
  ...
  if (!newActividad.value.trabajo) {
    $q.notify({
      type: "negative",
      message: "Por favor completa todos los campos de la actividad.",
    });
    return false;
  }
  ...
  for (const fecha of fechaHoraList.value) {
    if (!fecha.fechaInicio || !fecha.timeE || !fecha.timeS) {
      $q.notify({
        type: "negative",
        message: "Por favor completa todos los campos de fecha y horario.",
      });
      return false;
    }
  }
  ...
  for (const operador of operadoresList.value) {
    if (!operador.nombre || !operador.rol) {
      $q.notify({
        type: "negative",
        message: "Por favor completa todos los datos de los operadores.",
      });
      return false;
    }
  }
  ...
  for (const doc of documentosList.value) {
    if (!doc.titulo) {
      $q.notify({
        type: "negative",
        message: "Por favor completa el título de todos los documentos.",
      });
      return false;
    }
    if (doc.cargarDetalles && (!doc.url || !doc.dataTang)) {
      $q.notify({
        type: "negative",
        message: "Por favor completa los detalles de los documentos marcados.",
      });
      return false;
    }
  }
...
const registrarActividad = async () => {
  ...
  fechaHoraList.value = fechaHoraList.value
    .map((item) => {
...
const confirmarPREV = async (index) => {
  try {
    ...
    console.error(
      "Error al confirmar la prevención:",
...
const denegarPREV = async (index) => {
  try {
    ...
    console.error(
      "Error al denegar la prevención:",
      error.response?.data || error.message
    );
  }
};
...
onMounted(() => {
  ...
  echo.channel("jobs-channel").listen(".job-updated", (data) => {
